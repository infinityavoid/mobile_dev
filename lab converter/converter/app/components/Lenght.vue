<template>
    ...
    <StackLayout >
            <StackLayout backgroundColor="#d4d8d4">
                <FlexboxLayout>
                    <label > Из: </label>
                    <label  v-if = "status"> Дюймы </label>
                    <label  v-else> Сантиметры </label>
                </FlexboxLayout>
                <FlexboxLayout>
                    <label > В: </label>
                    <label  v-if = "status"> Сантиметры </label>
                    <label  v-else> Дюймы </label>
                </FlexboxLayout>
            </StackLayout>
            <FlexboxLayout >
   ...
   methods:{
    ...
    Convert: function(num)
    {
        if(this.status == true)
            ...
            {
                this.textFieldValue += num;
                this.Changed = this.textFieldValue * this.dum;
            }
            catch
            {
        ...
        else
        {
            ...
            {
                this.textFieldValue += num;
                this.Changed = this.textFieldValue / this.sm;
            }
            catch
            {
                alert('Данные введены не корректно')
