<template>
    ...
    <StackLayout >
            <StackLayout backgroundColor="#d4d8d4">
                <FlexboxLayout>
                    <label > Из: </label>
                    <label  v-if = "status"> Зиверты </label>
                    <label  v-else> Рентгены </label>
                </FlexboxLayout>
                <FlexboxLayout>
                    <label > В: </label>
                    <label  v-if = "status"> Рентгены </label>
                    <label  v-else> Зиверты </label>
                </FlexboxLayout>
            </StackLayout>
            <FlexboxLayout >
   ...
   methods:{
    ...
    Convert: function(num)
    {
        if(this.status == true)
            ...
            {
                this.textFieldValue += num;
                this.Changed = this.textFieldValue * this.Ren;
            }
            catch
            {
        ...
        else
        {
            ...
            {
                this.textFieldValue += num;
                this.Changed = this.textFieldValue / this.Ziv;
            }
            catch
            {
                alert('Данные введены не корректно')
