<template>
    ...
    <StackLayout >
            <StackLayout backgroundColor="#d4d8d4">
                <FlexboxLayout>
                    <label > Из: </label>
                    <label  v-if = "status"> Радианы </label>
                    <label  v-else> Градусы </label>
                </FlexboxLayout>
                <FlexboxLayout>
                    <label > В: </label>
                    <label  v-if = "status"> Градусы </label>
                    <label  v-else> Радианы </label>
                </FlexboxLayout>
            </StackLayout>
            <FlexboxLayout >
   ...
   methods:{
    ...
    Convert: function(num)
    {
        if(this.status == true)
            ...
            {
                this.textFieldValue += num;
                this.Changed = this.textFieldValue * this.Rad;
            }
            catch
            {
        ...
        else
        {
            ...
            {
                this.textFieldValue += num;
                this.Changed = this.textFieldValue / this.Gr;
            }
            catch
            {
                alert('Данные введены не корректно')
